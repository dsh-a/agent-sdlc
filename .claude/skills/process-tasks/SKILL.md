...
- **Completion protocol**:
    ...
    ii. If all sub-tasks underneath a parent task are now `[x]`, follow this sequence:
        - **First**: Run the full test suite (using the test command from **Project Commands** in `.claude/config.md`), pause and notify the user of the results: "Tests passed" or "Tests failed" before proceeding.
